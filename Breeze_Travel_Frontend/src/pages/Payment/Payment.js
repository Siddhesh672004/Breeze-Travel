...
export const Payment = () => {
    ...
    (async () => {
      try {
        const { data } = await axios.get(
          `https://breeze-travel-u1pm.onrender.com/api/hotels/${id}`
        );
        setSingleHotel(data);
      } catch (err) {
        console.log(err);
  ...
  const totalPayableAmount = price * numberOfNights;
  ...
  const handleConfirmBookingClick = async () => {
    ...
    console.log(process.env.REACT_APP_API_KEY, "process.env.key");
    const options = {
